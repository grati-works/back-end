...
describe('Remove Reaction', () => {
  ...
  it('should be able to remove reaction to a message', async () => {
    ...
    const removeReactionUseCaseSpy = jest.spyOn(
    ...
    await removeReactionUseCase.execute(
      senderUser.id.toString(),
      feedbackId.toString(),
      'thumbsup',
    );

    expect(removeReactionUseCaseSpy).toHaveBeenCalledWith(
      senderUser.id.toString(),
      feedbackId.toString(),
      'thumbsup',
    );
  ...
  it('should not be able to remove reaction to a message if the user is inexistent', async () => {
    await expect(
      removeReactionUseCase.execute('9418', '123', 'thumbsup'),
    ).rejects.toEqual(new AppError('User not found', 404, 'user.not_found'));
  });

  it('should not be able to remove reaction to a message if the message is inexistent', async () => {
    const { createdUser: senderUser } = await createFakeProfile();

    await expect(
      removeReactionUseCase.execute(
        senderUser.id.toString(),
        '123',
        'thumbsup',
      ),
